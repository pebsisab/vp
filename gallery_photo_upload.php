<?php
	...
	//kontrollin pildi valikut (kas vormi ülekande meetod oli post)
	$file_type = null;
	$photo_error = null;
	$photo_file_size_limit = 1.5 * 1024 * 1024;
	$photo_name_prefix = "vp_";
	$file_name = null;
	$normal_photo_max_w = 800; //max pildi laius
	$normal_photo_max_h = 450; //max pildi kõrgus
	
	if($_SERVER["REQUEST_METHOD"] == "POST"){
		if(isset($_POST["photo_submit"])){
			//var_dump($_POST);
			//var_dump($_FILES["photo_input"]);
			//kas on üldse olemas pildifail ja mis tüüp
			...
			if(empty($photo_error)){
				if($_FILES["photo_input"]["size"] > $photo_file_size_limit){
					$photo_error = "valitud pilt on liiga suur";
				}	
			}
			...
			if(empty($photo_error)){
				...
				//loome pixlikogumi e. image objekti
