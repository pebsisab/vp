	...
	function sign_in($email, $password){
        ...
        if($stmt->fetch()){ //teeb tulemused prepared olekusse
            ...
            if(password_verify($password, $password_from_db)){
				...
				$stmt->execute();
				if($stmt->fetch()){
					//parool õige, määrame sessioonimuutujad
					...
					$_SESSION["user_txt_color"] = "#0000CC";
					//värvide profiilist lugemine, kui on, tulevad uued väärtused. Kui pole, jäävad need, mis on kirjas
					$stmt->close();
					...
					$login_error = "Sisselogimisel tekkis viga!";
				}
            } else {
